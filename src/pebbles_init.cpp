...
#include "pebbles_server_status.h"
#include "pebbles_parameters.h"

namespace mongo {
    const std::string kLevelDBEngineName = "pebblesdb";

    namespace {

        class LevelFactory : public StorageEngine::Factory {
        public:
            ...
            virtual StorageEngine* create(const StorageGlobalParams& params,
                ...
                // db/ directory
                if (formatVersion == -1) {
                    // it's a new database, set it to the newest leveldb version kLevelFormatVersion
                    formatVersion = kLevelFormatVersion;
                }
                auto engine = new LevelEngine(params.dbpath + "/db", params.dur, formatVersion,
                                              params.readOnly);
                // Intentionally leaked.
                auto leaked __attribute__((unused)) = new LevelServerStatusSection(engine);
                auto leaked2 __attribute__((unused)) = new LevelRateLimiterServerParameter(engine);
                auto leaked3 __attribute__((unused)) = new LevelBackupServerParameter(engine);
                auto leaked4 __attribute__((unused)) = new LevelCompactServerParameter(engine);
                auto leaked5 __attribute__((unused)) = new LevelCacheSizeParameter(engine);
                auto leaked6 __attribute__((unused)) = new LevelOptionsParameter(engine);

                return new KVStorageEngine(engine, options);
            }

            virtual StringData getCanonicalName() const {
                return kLevelDBEngineName;
            }

            virtual Status validateMetadata(const StorageEngineMetadata& metadata,
                                            const StorageGlobalParams& params) const {
                const BSONObj& options = metadata.getStorageEngineOptions();
                BSONElement element = options.getField(kLevelFormatVersionString);
                ...
                if (element.numberInt() < kMinSupportedLevelFormatVersion) {
                    ...
                    return Status(
                        ...
                        str::stream()
                            ...
                            << kMinSupportedLevelFormatVersion << " to " << kLevelFormatVersion
                            << ". Please reload the database using mongodump and mongorestore");
                } else if (element.numberInt() > kLevelFormatVersion) {
            ...
            virtual BSONObj createMetadataOptions(const StorageGlobalParams& params) const {
                ...
                builder.append(kLevelFormatVersionString, kLevelFormatVersion);
                return builder.obj();
            }
        ...
        private:
            ...
            const int kLevelFormatVersion = 3;
            const int kMinSupportedLevelFormatVersion = 2;
            const std::string kLevelFormatVersionString = "levelFormatVersion";
            int mutable formatVersion = -1;
        };
    } // namespace

    MONGO_INITIALIZER_WITH_PREREQUISITES(LevelEngineInit,
                                         ("SetGlobalEnvironment"))
                                         (InitializerContext* context) {
        getGlobalServiceContext()->registerStorageEngine(kLevelDBEngineName, new LevelFactory());
        return Status::OK();
    }
