...
#include "hyperlevel_engine.h"
#include "hyperlevel_index.h"
#include "hyperlevel_recovery_unit.h"
#include "hyperlevel_transaction.h"
#include "hyperlevel_snapshot_manager.h"
...
namespace {
    ...
    class LevelIndexHarness final : public SortedDataInterfaceHarnessHelper {
    public:
        LevelIndexHarness() : _order(Ordering::make(BSONObj())), _tempDir(_testNamespace) {
            ...
            ASSERT(s.ok());
            _db.reset(db);
            _counterManager = stdx::make_unique<LevelCounterManager>(_db.get(), true);
        ...
        std::unique_ptr<SortedDataInterface> newSortedDataInterface(bool unique) {
            ...
            LevelIndexBase::generateConfig(&configBuilder, 3, IndexDescriptor::IndexVersion::kV2);
            if (unique) {
                return stdx::make_unique<LevelUniqueIndex>(_db.get(), "prefix", "ident", _order,
                                                           configBuilder.obj());
            } else {
                return stdx::make_unique<LevelStandardIndex>(_db.get(), "prefix", "ident", _order,
                                                             configBuilder.obj());
            }
        }
    ...
    private:
        Ordering _order;
        string _testNamespace = "mongo-level-sorted-data-test";
    ...
    std::unique_ptr<HarnessHelper> makeHarnessHelper() {
        return stdx::make_unique<LevelIndexHarness>();
    }
    ...
    TEST(LevelIndexTest, Isolation) {
        auto harnessHelper = stdx::make_unique<LevelIndexHarness>();
    ...
    void testSeekExactRemoveNext(bool forward, bool unique) {
        std::unique_ptr<SortedDataInterfaceHarnessHelper> harnessHelper =
            stdx::make_unique<LevelIndexHarness>();
    ...
    TEST(LevelIndexTest, SeekExactRemoveNext_Forward_Unique) {
        testSeekExactRemoveNext(true, true);
    }

    TEST(LevelIndexTest, SeekExactRemoveNext_Forward_Standard) {
        testSeekExactRemoveNext(true, false);
    }

    TEST(LevelIndexTest, SeekExactRemoveNext_Reverse_Unique) {
        testSeekExactRemoveNext(false, true);
    }

    TEST(LevelIndexTest, SeekExactRemoveNext_Reverse_Standard) {
