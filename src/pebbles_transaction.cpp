...
namespace mongo {
    LevelTransactionEngine::LevelTransactionEngine()
        : _latestSnapshotId(1),
    ...
    size_t LevelTransactionEngine::numKeysTracked() {
        stdx::lock_guard<stdx::mutex> lk(_lock);
        return _keyInfo.size();
    }
    size_t LevelTransactionEngine::numActiveSnapshots() {
        stdx::lock_guard<stdx::mutex> lk(_lock);
        return _activeSnapshots.size();
    }

    std::list<uint64_t>::iterator LevelTransactionEngine::_getLatestSnapshotId_inlock() {
        return _activeSnapshots.insert(_activeSnapshots.end(), _latestSnapshotId);
    }

    bool LevelTransactionEngine::_isKeyCommittedAfterSnapshot_inlock(const std::string& key,
                                                                     uint64_t snapshotId) {
        auto iter = _keyInfo.find(key);
        return iter != _keyInfo.end() && iter->second.first > snapshotId;
    }

    void LevelTransactionEngine::_registerCommittedKey_inlock(const std::string& key,
    ...
    void LevelTransactionEngine::_cleanUpKeysCommittedBeforeSnapshot_inlock(uint64_t snapshotId) {
    ...
    void LevelTransactionEngine::_cleanupSnapshot_inlock(
    ...
    bool LevelTransaction::registerWrite(const std::string& key) {
        stdx::lock_guard<stdx::mutex> lk(_transactionEngine->_lock);
    ...
    void LevelTransaction::recordSnapshotId() {
        {
            stdx::lock_guard<stdx::mutex> lk(_transactionEngine->_lock);
            _cleanup_inlock();
    ...
    void LevelTransaction::_cleanup_inlock() {
