...
namespace mongo {
    ...
    class LevelIndexBase;
    class LevelRecordStore;
    class JournalListener;

    class LevelEngine final : public KVEngine {
        MONGO_DISALLOW_COPYING( LevelEngine );
    public:
        LevelEngine(const std::string& path, bool durable, int formatVersion, bool readOnly);
        virtual ~LevelEngine();
	...
	virtual int flushAllFiles(bool sync) override;
        ...
        leveldb::DB* getDB() { return _db.get(); }
        const leveldb::DB* getDB() const { return _db.get(); }
        leveldb::Cache* getBlockCache() { return _block_cache; }
        std::unordered_set<uint32_t> getDroppedPrefixes() const;

        LevelTransactionEngine* getTransactionEngine() { return &_transactionEngine; }
    ...
    private:
        ...
        leveldb::Options _options() const;

        std::string _path;
        std::unique_ptr<leveldb::DB> _db;
        leveldb::Cache* _block_cache;
        ...
        StringMap<LevelIndexBase*> _identIndexMap;
        StringMap<LevelRecordStore*> _identCollectionMap;
        ...
        std::unique_ptr<LevelCompactionScheduler> _compactionScheduler;
        ...
        class LevelJournalFlusher;
        std::unique_ptr<LevelJournalFlusher> _journalFlusher;  // Depends on _durabilityManager
    };
