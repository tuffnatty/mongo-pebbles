...
namespace mongo {
    ...
    class LevelRecordStoreHarnessHelper final : public RecordStoreHarnessHelper {
    public:
        LevelRecordStoreHarnessHelper() : _tempDir(_testNamespace) {
            boost::filesystem::remove_all(_tempDir.path());
            leveldb::DB* db;
            leveldb::Options options;
            options.create_if_missing = true;
            auto s = leveldb::DB::Open(options, _tempDir.path(), &db);
            ASSERT(s.ok());
            _db.reset(db);
            _counterManager.reset(new LevelCounterManager(_db.get(), true));
            _durabilityManager.reset(new LevelDurabilityManager(_db.get(), true));
        }
        ...
        std::unique_ptr<RecordStore> newNonCappedRecordStore(const std::string& ns) {
            return stdx::make_unique<LevelRecordStore>(ns, "1", _db.get(), _counterManager.get(),
        ...
        std::unique_ptr<RecoveryUnit> newRecoveryUnit() final {
            return stdx::make_unique<LevelRecoveryUnit>(&_transactionEngine, &_snapshotManager,
    ...
    private:
        string _testNamespace = "mongo-level-record-store-test";
        unittest::TempDir _tempDir;
        std::unique_ptr<leveldb::DB> _db;
        LevelTransactionEngine _transactionEngine;
        LevelSnapshotManager _snapshotManager;
        std::unique_ptr<LevelDurabilityManager> _durabilityManager;
        std::unique_ptr<LevelCounterManager> _counterManager;
    };

    std::unique_ptr<HarnessHelper> makeHarnessHelper() {
        return stdx::make_unique<LevelRecordStoreHarnessHelper>();
    }
    ...
    TEST(LevelRecordStoreTest, Isolation1 ) {
        auto harnessHelper = stdx::make_unique<LevelRecordStoreHarnessHelper>();
    ...
    TEST(LevelRecordStoreTest, Isolation2 ) {
        auto harnessHelper = stdx::make_unique<LevelRecordStoreHarnessHelper>();
        std::unique_ptr<RecordStore> rs( harnessHelper->newNonCappedRecordStore() );
    ...
    StatusWith<RecordId> insertBSON(ServiceContext::UniqueOperationContext& opCtx,
        ...
        LevelRecordStore* rrs = dynamic_cast<LevelRecordStore*>(rs.get());
        invariant( rrs );
    ...
    TEST(LevelRecordStoreTest, OplogHack) {
        LevelRecordStoreHarnessHelper harnessHelper;
    ...
    TEST(LevelRecordStoreTest, DeleteSeekExactRecord_Forward_Capped) {
        testDeleteSeekExactRecord(true, true);
    }

    TEST(LevelRecordStoreTest, DeleteSeekExactRecord_Forward_NonCapped) {
        testDeleteSeekExactRecord(true, false);
    }

    TEST(LevelRecordStoreTest, DeleteSeekExactRecord_Reversed_Capped) {
        testDeleteSeekExactRecord(false, true);
    }

    TEST(LevelRecordStoreTest, DeleteSeekExactRecord_Reversed_NonCapped) {
        testDeleteSeekExactRecord(false, false);
    }

    TEST(LevelRecordStoreTest, OplogHackOnNonOplog) {
        LevelRecordStoreHarnessHelper harnessHelper;
        std::unique_ptr<RecordStore> rs(
    ...
    TEST(LevelRecordStoreTest, CappedOrder) {
        std::unique_ptr<LevelRecordStoreHarnessHelper> harnessHelper(
                new LevelRecordStoreHarnessHelper());
    ...
    TEST(LevelRecordStoreTest, OplogOrder) {
        std::unique_ptr<LevelRecordStoreHarnessHelper> harnessHelper(
            new LevelRecordStoreHarnessHelper());
        std::unique_ptr<RecordStore> rs(
            harnessHelper->newCappedRecordStore("local.oplog.foo", 100000, -1));
        {
            const LevelRecordStore* rrs = dynamic_cast<LevelRecordStore*>(rs.get());
            ASSERT( rrs->isOplog() );
        }
