...
namespace mongo {
    ...
    class LevelCounterManager;
    class LevelDurabilityManager;
    class LevelRecoveryUnit;
    class LevelOplogKeyTracker;
    class LevelRecordStore;

    typedef std::list<RecordId> SortedRecordIds;

    class CappedVisibilityManager {
    public:
        CappedVisibilityManager(LevelRecordStore* rs, LevelDurabilityManager* durabilityManager);
        ...
        void oplogJournalThreadLoop(LevelDurabilityManager* durabilityManager);
    ...
    class LevelRecordStore : public RecordStore {
    public:
        LevelRecordStore(StringData ns, StringData id, leveldb::DB* db,
                         LevelCounterManager* counterManager,
                         LevelDurabilityManager* durabilityManager, std::string prefix,
                         bool isCapped = false, int64_t cappedMaxSize = -1,
                         int64_t cappedMaxDocs = -1, CappedCallback* cappedDeleteCallback = NULL);

        virtual ~LevelRecordStore();

        // name of the RecordStore implementation
        virtual const char* name() const { return "level"; }
        ...
        static leveldb::Comparator* newLevelCollectionComparator();

        class CappedInsertChange;
    private:
        friend class CappedVisibilityManager;
        // we just need to expose _makePrefixedKey to LevelOplogKeyTracker
        friend class LevelOplogKeyTracker;
        // NOTE: Cursor might outlive the RecordStore. That's why we use all those
        // shared_ptrs
        class Cursor : public SeekableRecordCursor {
        public:
            Cursor(OperationContext* txn, leveldb::DB* db, std::string prefix,
        ...
        private:
            ...
            RecordId _lastLoc;
            std::unique_ptr<leveldb::Iterator> _iterator;
            std::string _seekExactResult;
            void positionIterator();
            leveldb::Iterator* iterator();
        };

        static RecordId _makeRecordId( const leveldb::Slice& slice );

        static RecordData _getDataFor(leveldb::DB* db, const std::string& prefix,
        ...
        static leveldb::Slice _makeKey(const RecordId& loc, int64_t* storage);
        ...
        LevelOplogKeyTracker* _oplogKeyTracker;
