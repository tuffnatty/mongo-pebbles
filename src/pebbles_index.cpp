...
namespace mongo {
    ...
    namespace {
        ...
        Status checkKeySize(const BSONObj& key) {
            if (key.objsize() >= kTempKeyMaxSize) {
                string msg = mongoutils::str::stream()
                             << "LevelIndex::insert: key too large to index, failing " << ' '
                             << key.objsize() << ' ' << key;
         ...
         */
        class LevelCursorBase : public SortedDataInterface::Cursor {
        public:
            LevelCursorBase(OperationContext* txn, leveldb::DB* db, std::string prefix,
        ...
        protected:
            ...
            void advanceCursor() {
                ...
                if (_iterator.get() == nullptr) {
                    _iterator.reset(LevelRecoveryUnit::getLevelRecoveryUnit(_txn)
                            ->NewIterator(_prefix));
                    _iterator->SeekPrefix(leveldb::Slice(_key.getBuffer(), _key.getSize()));
            ...
            bool seekCursor(const KeyString& query) {
                auto * iter = iterator();
                const leveldb::Slice keySlice(query.getBuffer(), query.getSize());
                iter->Seek(keySlice);
            ...
            leveldb::Slice _valueSlice() {
                if (_iterator.get() == nullptr) {
                    return leveldb::Slice(_value);
                }
                return leveldb::Slice(_iterator->value());
            }

            leveldb::DB* _db;                                       // not owned
            std::string _prefix;
            std::unique_ptr<LevelIterator> _iterator;
            const bool _forward;
        ...
        class LevelStandardCursor final : public LevelCursorBase {
        public:
            LevelStandardCursor(OperationContext* txn, leveldb::DB* db, std::string prefix,
                                bool forward, Ordering order, KeyString::Version keyStringVersion)
                : LevelCursorBase(txn, db, prefix, forward, order, keyStringVersion) {
                iterator();
            }
        ...
        class LevelUniqueCursor final : public LevelCursorBase {
        public:
            LevelUniqueCursor(OperationContext* txn, leveldb::DB* db, std::string prefix,
                              bool forward, Ordering order, KeyString::Version keyStringVersion)
                : LevelCursorBase(txn, db, prefix, forward, order, keyStringVersion) {}
     ...
     */
    class LevelIndexBase::StandardBulkBuilder : public SortedDataBuilderInterface {
    public:
        StandardBulkBuilder(LevelStandardIndex* index, OperationContext* txn) : _index(index),
                                                                                _txn(txn) {}
     ...
     * append data.
     */
    class LevelIndexBase::UniqueBulkBuilder : public SortedDataBuilderInterface {
    public:
    ...
    private:
        void doInsert() {
            ...
            std::string prefixedKey(LevelIndexBase::_makePrefixedKey(_prefix, _keyString));
            leveldb::Slice valueSlice(value.getBuffer(), value.getSize());

            auto ru = LevelRecoveryUnit::getLevelRecoveryUnit(_txn);
	    ru->writeBatch()->Put(prefixedKey, valueSlice);
	    ru->writeBatchRepHandler()->Add(prefixedKey, valueSlice);
    ...
    LevelIndexBase::LevelIndexBase(leveldb::DB* db, std::string prefix, std::string ident,
          ...
          _order(order)
    {
        ...
        std::string nextPrefix = levelGetNextPrefix(_prefix);
        leveldb::Range wholeRange(_prefix, nextPrefix);
    ...
    void LevelIndexBase::fullValidate(OperationContext* txn, long long* numKeysOut,
    ...
    bool LevelIndexBase::isEmpty(OperationContext* txn) {
        auto ru = LevelRecoveryUnit::getLevelRecoveryUnit(txn);
        std::unique_ptr<leveldb::Iterator> it(ru->NewIterator(_prefix));

        it->SeekToFirst();
        return !it->Valid();
    }

    Status LevelIndexBase::initAsEmpty(OperationContext* txn) {
        // no-op
        return Status::OK();
    }

    long long LevelIndexBase::getSpaceUsedBytes(OperationContext* txn) const {
    ...
    void LevelIndexBase::generateConfig(BSONObjBuilder* configBuilder, int formatVersion,
    ...
    std::string LevelIndexBase::_makePrefixedKey(const std::string& prefix,
    ...
    LevelUniqueIndex::LevelUniqueIndex(leveldb::DB* db, std::string prefix, std::string ident,
                                       Ordering order, const BSONObj& config)
        : LevelIndexBase(db, prefix, ident, order, config) {}

    Status LevelUniqueIndex::insert(OperationContext* txn, const BSONObj& key, const RecordId& loc,
                                    bool dupsAllowed) {
	...
	auto getStatus = ru->Get(prefixedKey, &currentValue);
        if (!getStatus.ok() && !getStatus.IsNotFound()) {
            return levelToMongoStatus(getStatus);
        ...
        leveldb::Slice valueVectorSlice(valueVector.getBuffer(), valueVector.getSize());
	ru->writeBatch()->Put(prefixedKey, valueVectorSlice);
        ru->writeBatchRepHandler()->Add(prefixedKey, valueVectorSlice);
        return Status::OK();
    }

    void LevelUniqueIndex::unindex(OperationContext* txn, const BSONObj& key, const RecordId& loc,
                                   bool dupsAllowed) {
	  ...
	  ru->writeBatchRepHandler()->Delete(prefixedKey);	  
	  return;
        }
	...
	auto getStatus = ru->Get(prefixedKey, &currentValue);
        ...
        leveldb::Slice newValueSlice(newValue.getBuffer(), newValue.getSize());
	ru->writeBatch()->Put(prefixedKey, newValueSlice);
        ru->writeBatchRepHandler()->Add(prefixedKey, newValueSlice);
    }

    std::unique_ptr<SortedDataInterface::Cursor> LevelUniqueIndex::newCursor(OperationContext* txn,
                                                                             bool forward) const {
        return stdx::make_unique<LevelUniqueCursor>(txn, _db, _prefix, forward, _order,
                                                    _keyStringVersion);
    }

    Status LevelUniqueIndex::dupKeyCheck(OperationContext* txn, const BSONObj& key,
                                         const RecordId& loc) {
        KeyString encodedKey(_keyStringVersion, key, _order);
        std::string prefixedKey(_makePrefixedKey(_prefix, encodedKey));
    ...
    SortedDataBuilderInterface* LevelUniqueIndex::getBulkBuilder(OperationContext* txn,
                                                                 bool dupsAllowed) {
        return new LevelIndexBase::UniqueBulkBuilder(_prefix, _order, _keyStringVersion, txn,
                                                     dupsAllowed);
    }

    /// LevelStandardIndex
    LevelStandardIndex::LevelStandardIndex(leveldb::DB* db, std::string prefix, std::string ident,
                                           Ordering order, const BSONObj& config)
        : LevelIndexBase(db, prefix, ident, order, config),
          useSingleDelete(false) {}

    Status LevelStandardIndex::insert(OperationContext* txn, const BSONObj& key,
        ...
        if (!encodedKey.getTypeBits().isAllZeros()) {
            value =
                leveldb::Slice(reinterpret_cast<const char*>(encodedKey.getTypeBits().getBuffer()),
	...
	ru->writeBatch()->Put(prefixedKey, value);
        ru->writeBatchRepHandler()->Add(prefixedKey, value);

        return Status::OK();
    }

    void LevelStandardIndex::unindex(OperationContext* txn, const BSONObj& key, const RecordId& loc,
                                     bool dupsAllowed) {
        invariant(dupsAllowed);
        // When DB parameter failIndexKeyTooLong is set to false,
        // this method may be called for non-existing
    ...
    std::unique_ptr<SortedDataInterface::Cursor> LevelStandardIndex::newCursor(
            OperationContext* txn,
            bool forward) const {
        return stdx::make_unique<LevelStandardCursor>(txn, _db, _prefix, forward, _order,
                                                      _keyStringVersion);
    }

    SortedDataBuilderInterface* LevelStandardIndex::getBulkBuilder(OperationContext* txn,
                                                                   bool dupsAllowed) {
        invariant(dupsAllowed);
        return new LevelIndexBase::StandardBulkBuilder(this, txn);
    }
