...
namespace mongo {
    ...
    long long LevelCounterManager::loadCounter(const std::string& counterKey) {
        {
        ...
        std::string value;
        auto s = _db->Get(leveldb::ReadOptions(), counterKey, &value);
        if (s.IsNotFound()) {
            return 0;
        }
        invariantLevelOK(s);
    ...
    void LevelCounterManager::updateCounter(const std::string& counterKey, long long count,
                                            leveldb::WriteBatch* writeBatch, leveldb::WriteBatchRepHandler *writeBatchRepHandler) {

        if (_crashSafe) {
            int64_t storage;
	    writeBatch->Put(counterKey, _encodeCounter(count, &storage));
	    writeBatchRepHandler->Add(counterKey, _encodeCounter(count, &storage));
	} else {
            stdx::lock_guard<stdx::mutex> lk(_lock);
		  ...
		  writeBatchRepHandler->Add(counter.first, _encodeCounter(counter.second, &storage));		    
		}
                _counters.clear();
                _syncCounter = 0;
            }
	...
	leveldb::WriteBatchRepHandler wbrh;
	{
	      ...
	      wbrh.Add(counter.first, _encodeCounter(counter.second, &storage));
            }
            _counters.clear();
            _syncCounter = 0;
            _syncing = true;
    ...
    leveldb::Slice LevelCounterManager::_encodeCounter(long long counter, int64_t* storage) {
